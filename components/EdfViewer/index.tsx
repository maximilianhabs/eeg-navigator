...
// ── Single canvas panel (one EDF file) ───────────────────────────────────────

function EdfPanel({
  example, montage, gain, windowSec,
}: {
  example: EdfExample
  montage: MontageId
  gain: number
  ...
  }, [header, signals, montage, gain, viewStart, windowSec, duration])
  ...
  return (
    <div style={{ borderTop: '1px solid var(--border)' }}>
      {/* Per-example mini toolbar: navigation + metadata */}
  ...
  return (
    <div>
      ...
      <div className="rounded-2xl border overflow-hidden"
        ...
        {/* Stacked panels */}
        {examples.map(ex => (
          <EdfPanel key={ex.filename} example={ex} montage={montage} gain={gain} windowSec={windowSec} />
        ))}
      </div>
    </div>
  )
